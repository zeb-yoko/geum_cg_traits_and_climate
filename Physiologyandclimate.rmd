...
```{r}
rpt.f <- rptGaussian(formula = Chlorophyll.Content ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = chl.1, nboot = 0, ratio = T, adjusted = F)
rpt.f
```
...
```{r}
(rpt.s <- rptGaussian(formula = SLA ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = sla.1, nboot = 0, ratio = T, adjusted = F))
...
pull out the important components, particularly variance and CV
```{r}
(rpt.l <- rptGaussian(formula = Leaf.Dry.Matter.Content ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = sla.1, nboot = 0, ratio = T, adjusted = F))
...
pull out the important components, particularly variance and CV
```{r}
(rpt.c <- rptGaussian(formula = Conductance ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = cond, nboot = 0, ratio = T, adjusted = F))
...
pull out the important components, particularly variance and CV
```{r}
(rpt.db <- rptGaussian(formula = Density_B ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = sto.2, nboot = 0, npermut = 1000, ratio = T, adjusted = F))
```
...
pull out the important components, particularly variance and CV
```{r}
(rpt.dt <- rptGaussian(formula = Density_T ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = sto.2, nboot = 0, ratio = T, adjusted = F))
```
...
pull out the important components, particularly variance and CV
```{r}
(rpt.rb <- rptGaussian(formula = Av_GCLengthB ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = stoSizeB, nboot = 0, ratio = T, adjusted = F))
```
...
pull out the important components, particularly variance and CV
```{r}
(rpt.rt <- rptGaussian(formula = Av_GCLengthT ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = stoSizeT, nboot = 0, ratio = T, adjusted = F))
```
...
pull out the important components, particularly variance and CV
```{r}
(rpt.aib <- rptGaussian(formula = SAI_B ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = sto.aib, nboot = 0, ratio = T, adjusted = F))
...
pull out the important components, particularly variance and CV
```{r}
(rpt.ait <- rptGaussian(formula = SAI_T ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = sto.ait, nboot = 0, ratio = T, adjusted = F))
```
...
pull out the important components, particularly variance and CV
```{r}
(rpt.cic <- rptGaussian(formula = dC13 ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = CIC, nboot = 0, ratio = T, adjusted = F))
```
...
pull out the important components, particularly variance and CV
```{r}
(rpt.sd <- rptGaussian(formula = Sinus_Depth ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = df1, nboot = 0, ratio = T, adjusted = F))
```
...
pull out the important components, particularly variance and CV
```{r}
(rpt.mv <- rptGaussian(formula = Mid_Vein_Length ~ Region + (1|Population), grname = c("Population", "Fixed", "Residual"), data = df1, nboot = 0, ratio = T, adjusted = F))
```
...
rpt.ml <- rptBinary(formula = Mini_Leaflets_YN ~ Region + (1|Population), 
						  ...
						  data = df1, link = "logit", nboot = 0, ratio = T, adjusted = F)
rpt.ml
...
##write table
```{r}
write.csv(prop.v, "Proportions of variancewa <- anova.csv", row.names =F)
```
...
df<- read.csv("Proportions of variancewa <- anova.csv")
df<-as.data.frame(df)
